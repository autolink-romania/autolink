...
function buildAgentPrompt(agent) {
  // Construiesc contextul mașinilor din garaj
  let carCtx = '';
  if (typeof cars !== 'undefined' && cars.length > 0) {
    carCtx = '\n\nGARAJUL UTILIZATORULUI:\n' + cars.map(c => {
      const docs = c.docs || {};
      const parts = [
        `• ${c.brand || ''} ${c.model || ''} ${c.year || ''}, nr. ${c.plate || 'necunoscut'}`,
        c.km ? `  Km: ${c.km}` : '',
        c.fuel ? `  Combustibil: ${c.fuel}` : '',
        c.engine ? `  Motor: ${c.engine}` : '',
        docs.rca ? `  RCA: expiră ${docs.rca}` : '  RCA: necunoscut',
        docs.itp ? `  ITP: expiră ${docs.itp}` : '  ITP: necunoscut',
        docs.rovinieta ? `  Rovinietă: expiră ${docs.rovinieta}` : '',
        c.vin ? `  VIN: ${c.vin}` : '',
      ].filter(Boolean).join('\n');
      return parts;
    }).join('\n\n');
    carCtx += '\n\nFolosește aceste date când utilizatorul întreabă despre mașinile lui. Nu cere informații pe care le ai deja.';
  } else {
    carCtx = '\n\nUtilizatorul nu are mașini adăugate în garaj momentan.';
  }

  const base = {
    manager: `Ești Agentul Manager al platformei AutoAssist. Coordonezi echipa de agenți (Auto, Documente, Juridic, Vânzare, ITP). Răspunzi la orice întrebare despre mașini, documente, legislație auto. Ești prietenos, profesionist și concis. Răspunzi în română.`,
  ...
  return (base[agent] || base.manager) + carCtx;
}

const AGENT_PROMPTS={
  manager:'',auto:'',documente:'',juridic:'',vanzare:'',itp:''
};
