...
function _applySection(sec){
  ...
  if(sec==='vanzare'){
    ...
    // Restaurez masina selectata si pozele dupa refresh
    setTimeout(()=>{
      const savedCarId = localStorage.getItem('vanz_sel_car_id');
      if(savedCarId && typeof selectVanzCar==='function'){
        selectVanzCar(parseInt(savedCarId));
        // Restaurez pozele din draft
        setTimeout(()=>{
...
function carLightbox(carId, startIdx) {
  ...
  document.body.insertAdjacentHTML('beforeend', `
    <div id="car-lb" style="position:fixed;inset:0;background:rgba(0,0,0,0.96);z-index:9999;display:flex;align-items:center;justify-content:center" onclick="if(event.target===this)document.getElementById('car-lb').remove()">
      ...
      <img id="car-lb-img" style="max-width:calc(100vw - 140px);max-height:88vh;border-radius:10px;object-fit:contain;display:block">
