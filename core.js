...
function _applySection(sec){
  ...
  if(sec==='asigurari') {
    setTimeout(resetRcaModal, 50);
    // Preîncarcă scripturile Safety Broker în background
    if (!document.getElementById('sb-modal-css')) {
      const css = document.createElement('link'); css.id='sb-modal-css'; css.rel='stylesheet'; css.href='https://insureframe.insuretech.ro/css/iframe-modal-core.css'; document.head.appendChild(css);
      const s1 = document.createElement('script'); s1.src='https://insureframe.insuretech.ro/js/iframe-modal-core.js'; document.body.appendChild(s1);
      const s2 = document.createElement('script'); s2.src='https://insureframe.insuretech.ro/js/iframe-communication.js'; document.body.appendChild(s2);
    }
  }
...
function isPremium() {
  ...
  if(currentUser.email === 'autoassist.romania@gmail.com') return true;
  if(currentUser.email === 'calin_nicole93@yahoo.com') return true;
