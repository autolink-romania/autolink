...
async function ocrTalon(input) {
  ...
  try {
    ...
    // Edge function poate returna reply, content sau message
    let reply = data.reply || '';
    if(!reply && data.content && Array.isArray(data.content)) {
      reply = data.content.map(b=>b.type==='text'?b.text:'').join('');
    }
    if(!reply && data.message) reply = data.message;
...
function addCar(){
  ...
  cars.push({
    ...
    fotos: (window._carFotos||[]).filter(Boolean)
  });
  save();
  // Salvează și în Supabase
  if(supabaseClient && currentUser) {
    ...
    supabaseClient.from('cars').upsert([{
      ...
      mnt: newCar.mnt,
      added: newCar.added
    }]).then(({error})=>{ if(error) console.error('Save car error:', error); });
  }
  closeM('add-car');renderAll();
  resetCarFotoPreviews();
}
...
function renderCarPage(car) {
  ...
  const mnt = car.mnt || {};
  // Calcul corect: (ultimul schimb + interval) - km actuali = km rămași
  // Dacă rezultatul e negativ = depășit
  const kmRamasi = (mnt.oilLast>=0 && mnt.oilInt) ? (mnt.oilLast + mnt.oilInt - (car.km||0)) : null;
  ...
  body.innerHTML = `
    ...
    <div style="background:linear-gradient(135deg,rgba(200,150,12,0.12),rgba(79,125,255,0.06));border:1px solid rgba(200,150,12,0.2);border-radius:16px;padding:20px;margin-bottom:20px">
      <div style="display:flex;justify-content:space-between;align-items:start;flex-wrap:wrap;gap:12px">
        ...
        <div style="text-align:right;display:flex;flex-direction:column;align-items:flex-end;gap:8px">
          ...
          <button class="btn btn-ghost btn-sm" onclick="openEditDocs(${car.id})" style="font-size:11px">✏️ Editează</button>
        </div>
      </div>
    ...
    <!-- Info vehicul -->
    <div class="card" style="margin-bottom:16px">
      <div class="ch"><span class="ct">ℹ️ Informații vehicul</span><button class="btn btn-ghost btn-sm" onclick="openEditDocs(${car.id})" style="font-size:11px">✏️ Editează</button></div>
...
function rDocs(){
  ...
  cars.forEach((c,ci)=>{
    ...
    const foto = c.fotos&&c.fotos[0] ? `<img src="${c.fotos[0]}" style="width:32px;height:32px;object-fit:cover;border-radius:6px;margin-right:8px;vertical-align:middle">` : '🚗 ';
    rows.push(`<tr><td colspan="5" style="padding:${ci>0?'20px':'8px'} 0 6px;border:none">
      <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:rgba(255,255,255,0.04);border-radius:10px;border-left:3px solid var(--accent)">
        <span style="font-size:20px">${c.fotos&&c.fotos[0]?'':'🚗'}</span>
        ${c.fotos&&c.fotos[0]?foto:''}
        <div>
          <div style="font-family:'Bebas Neue';letter-spacing:2px;font-size:18px;color:var(--gold)">${c.plate}</div>
          <div style="font-size:12px;color:var(--t2)">${c.brand} ${c.model}${c.year?' · '+c.year:''}${c.km?' · '+c.km.toLocaleString()+' km':''}</div>
        </div>
        <button onclick="openEditDocs(${c.id})" style="margin-left:auto;background:none;border:1px solid var(--b2);color:var(--t3);border-radius:8px;padding:4px 10px;cursor:pointer;font-size:11px">✏️ Editează</button>
      </div>
    </td></tr>`);
    Object.entries(c.docs).forEach(([k,v])=>{
      ...
      rows.push(`<tr>
        ...
        <td style="font-family:'JetBrains Mono';font-size:12px">${d!==null?(d<0?`<span style="color:var(--red);font-weight:700">Expirat (${Math.abs(d)}z)</span>`:`${d} zile`):'—'}</td>
        <td><span class="sts" style="background:${cm[cc]}18;color:${cm[cc]}">${im[cc]} ${lm[cc]}</span></td>
        <td>${act}</td>
      </tr>`);
    });
  });
...
function rVanzC(){
  ...
  // Verifică anunțurile active din localStorage
  const _anunturiActive = (() => { try { return JSON.parse(localStorage.getItem('vanz_anunturi')||'[]'); } catch(e){ return []; } })();

  el.innerHTML=`<div class="g3">${cars.map(c=>{
    const foto=c.fotos&&c.fotos[0]?c.fotos[0]:null;
    const anuntActiv = _anunturiActive.find(a=>a.plate===c.plate&&a.status==='activ');
    return`<div class="cc" style="padding:0;overflow:hidden;position:relative">
      ${anuntActiv?`<div style="position:absolute;top:8px;right:8px;z-index:2;background:var(--green);color:#000;font-size:10px;font-weight:800;padding:2px 8px;border-radius:10px">ACTIV</div>`:''}
      ${foto?`<div style="height:120px;overflow:hidden;cursor:pointer" onclick="openVanz(${c.id})"><img src="${foto}" style="width:100%;height:100%;object-fit:cover"></div>`:`<div style="height:80px;background:linear-gradient(135deg,rgba(79,125,255,0.1),rgba(200,150,12,0.06));display:flex;align-items:center;justify-content:center;font-size:36px;cursor:pointer" onclick="openVanz(${c.id})">🚗</div>`}
      <div style="padding:12px">
        <div class="cp">${c.plate}</div>
        <div class="cm">${c.brand} ${c.model} ${c.year||''} ${c.km?'· '+c.km.toLocaleString()+' km':''}</div>
        ${anuntActiv?`<div style="font-size:13px;font-weight:700;color:var(--gold);margin-top:6px">${anuntActiv.pret} EUR</div>`:''}
        <button class="btn btn-primary btn-full btn-sm" style="margin-top:10px" onclick="openVanz(${c.id})">💰 ${anuntActiv?'Modifică anunțul':'Publică anunț de vânzare'}</button>
      </div>
    </div>`;
  }).join('')}</div>`;
}
...
function isPremium() {
  ...
  if(email === 'calin.catalin.romania@gmail.com') return true;
...
function carLightbox(carId, startIdx) {
  const car = cars.find(c=>c.id==carId);
  if(!car||!car.fotos) return;
  const fotos = car.fotos.filter(Boolean);
  if(!fotos.length) return;
  let cur = startIdx < fotos.length ? startIdx : 0;

  const existing = car.fotos.map((f,i)=>f?{f,i}:null).filter(Boolean);
  cur = Math.min(cur, existing.length-1);

  document.getElementById('car-lb')?.remove();
  document.body.insertAdjacentHTML('beforeend', `
    <div id="car-lb" style="position:fixed;inset:0;background:rgba(0,0,0,0.96);z-index:9999;display:flex;align-items:center;justify-content:center" onclick="if(event.target===this)document.getElementById('car-lb').remove()">
      <button onclick="document.getElementById('car-lb').remove()" style="position:fixed;top:16px;right:16px;background:rgba(255,255,255,0.15);border:none;color:#fff;width:40px;height:40px;border-radius:50%;cursor:pointer;font-size:22px;z-index:10001">×</button>
  ...
  const update = () => {
    ...
    document.getElementById('car-lb-prev').style.opacity = window._carLbCur===0?'0.3':'1';
    document.getElementById('car-lb-next').style.opacity = window._carLbCur===window._carLbData.length-1?'0.3':'1';
  };
  window._carLbUpdate = update;
  update();
}

function carLbNav(dir) {
...
window._carFotos = [null, null];

function previewCarFoto(input, slot) {
  const file = input.files[0];
  if(!file) return;
  const reader = new FileReader();
  reader.onload = e => {
    window._carFotos[slot-1] = e.target.result;
    const img = document.getElementById('m-foto'+slot+'-img');
    const txt = document.getElementById('m-foto'+slot+'-text');
    if(img) { img.src = e.target.result; img.style.display='block'; }
    if(txt) txt.style.display='none';
    const lbl = document.getElementById('m-foto'+slot+'-label');
    if(lbl) lbl.style.borderColor='rgba(79,125,255,0.5)';
  };
  reader.readAsDataURL(file);
}

function resetCarFotoPreviews() {
  window._carFotos = [null, null];
  [1,2].forEach(i=>{
    const img = document.getElementById('m-foto'+i+'-img');
    const txt = document.getElementById('m-foto'+i+'-text');
    const lbl = document.getElementById('m-foto'+i+'-label');
    if(img) { img.src=''; img.style.display='none'; }
    if(txt) txt.style.display='';
    if(lbl) lbl.style.borderColor='rgba(255,255,255,0.15)';
  });
}
