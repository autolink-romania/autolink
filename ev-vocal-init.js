  ...
  setTimeout(function(){
    ...
    if(!document.querySelector('.sec.active')) {
      if(currentUser) {
        ...
        const validSections = ['dashboard','garaj','documente','rca','rovinieta','agenti','vocal','asistenta','mentenanta','anvelope','ev','vanzare','carvertical','legal','setari','docpers','servicii-stat','itp','costuri','verificare','peco','verif-nr','asistenta','premium'];
    ...
    try {
      ...
      if(code) {
        ...
        if(!error && data?.session?.user) {
          ...
          _applySection((()=>{const s=localStorage.getItem('aa_last_sec');const v=['dashboard','garaj','documente','rca','rovinieta','agenti','vocal','asistenta','mentenanta','anvelope','ev','vanzare','carvertical','legal','setari','docpers','servicii-stat','itp','costuri','verificare','peco','verif-nr','asistenta','premium'];return(s&&v.includes(s))?s:'dashboard';})());
          dismissOverlay();
          return;
        }
      }
...
async function loadUserData(userId) {
  ...
  try {
    ...
    if(data && data.length > 0) {
      cars = data.map(c => ({...c, docs: c.docs || {}, mnt: c.mnt || {oilInt:10000, oilLast:0}}));
