...
function toggleMic() {
  ...
  micActive = true;
  // Stop TTS când userul activează microfonul
  ...
  rec.onresult = function(e) {
    ...
    // Stop TTS când userul vorbește
    if(window.speechSynthesis) window.speechSynthesis.cancel();
    if(window._ttsAudio) { window._ttsAudio.pause(); window._ttsAudio.currentTime=0; }

    // Auto-send după 2.5 secunde de inactivitate
    clearTimeout(_voiceTimer);
    _voiceTimer = setTimeout(() => {
      if(_lastTranscript) {
        askVoice(_lastTranscript);
        _lastTranscript = '';
      }
    }, 2500);
  };
...
async function loadUserData(userId) {
  ...
  try {
    ...
    if(data && data.length > 0) {
      // Merge Supabase + localStorage (păstrăm mașinile locale care nu sunt în Supabase)
      ...
      const onlyLocal = localCars.filter(l=>!supabaseIds.has(String(l.id)));
      cars = [...supabaseCars, ...onlyLocal];
      // Sincronizez mașinile locale în Supabase
      for(const lc of onlyLocal) {
        supabaseClient.from('cars').upsert({...lc, user_id: userId}).then(({error})=>{
