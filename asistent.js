...
function showAnuntModal(a) {
  ...
  const esteAlMeu = a.user_id === currentUser?.id;
  const html = `
    <div style="position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px" onclick="this.remove()">
      <div style="background:var(--s1);border:1px solid var(--b2);border-radius:20px;max-width:500px;width:100%;max-height:90vh;overflow-y:auto" onclick="event.stopPropagation()">
        <div style="height:200px;background:var(--s2);border-radius:20px 20px 0 0;overflow:hidden;position:relative">
          ${a.foto?`<img src="${a.foto}" style="width:100%;height:100%;object-fit:cover">`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;font-size:72px">🚗</div>`}
          <div style="position:absolute;bottom:12px;left:12px;background:rgba(0,0,0,0.75);color:var(--gold);font-size:20px;font-weight:800;padding:6px 14px;border-radius:10px">${a.pret} EUR</div>
          <button onclick="this.closest('[style*=fixed]').remove()" style="position:absolute;top:12px;right:12px;background:rgba(0,0,0,0.6);border:none;color:#fff;width:32px;height:32px;border-radius:50%;cursor:pointer;font-size:18px">×</button>
        </div>
        <div style="padding:20px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px;font-size:13px">
            ...
            ${a.vin?`<div style="padding:8px;background:var(--s2);border-radius:8px;font-family:monospace">VIN: ${a.vin}</div>`:''}
          </div>
          ...
          ${esteAlMeu?`<button onclick="this.closest('[style*=fixed]').remove()" style="width:100%;margin-top:8px;background:var(--s2);border:1px solid var(--b2);color:var(--t2);padding:10px;border-radius:10px;cursor:pointer;font-size:13px">Acesta e anunțul tău</button>`:''}
        </div>
      </div>
    </div>`;
