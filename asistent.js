...
function showAnuntModal(a) {
  ...
  const fotoSection = fotos.length
    ? `<div style="position:relative;background:#000;border-radius:20px 20px 0 0;overflow:hidden">
        <div style="height:240px;display:flex;align-items:center;justify-content:center;cursor:pointer" onclick="anuntLbOpen()">
          <img id="anunt-modal-img" src="${fotos[0]}" style="max-width:100%;max-height:240px;object-fit:contain">
        </div>
        ${fotos.length>1?`
          <button onclick="anuntModalNav(-1)" style="position:absolute;left:8px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);border:none;color:#fff;width:38px;height:38px;border-radius:50%;cursor:pointer;font-size:22px">‹</button>
          <button onclick="anuntModalNav(1)" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:rgba(0,0,0,0.6);border:none;color:#fff;width:38px;height:38px;border-radius:50%;cursor:pointer;font-size:22px">›</button>
          <div id="anunt-modal-count" style="position:absolute;bottom:8px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.6);color:#fff;font-size:11px;padding:2px 10px;border-radius:10px">1/${fotos.length}</div>
        `:''}
        <div style="position:absolute;bottom:12px;left:12px;background:rgba(0,0,0,0.8);color:var(--gold);font-size:18px;font-weight:800;padding:5px 12px;border-radius:8px">${a.pret} EUR</div>
      </div>`
    : `<div style="height:100px;background:var(--s2);border-radius:20px 20px 0 0;display:flex;align-items:center;justify-content:center;font-size:56px;position:relative">
        🚗<div style="position:absolute;bottom:10px;left:12px;background:rgba(0,0,0,0.75);color:var(--gold);font-size:18px;font-weight:800;padding:5px 12px;border-radius:8px">${a.pret} EUR</div>
      </div>`;

  const html = `
    <div id="mo-anunt" style="position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px" onclick="if(event.target===this)document.getElementById('mo-anunt').remove()">
      <div style="background:var(--s1);border:1px solid var(--b2);border-radius:20px;max-width:500px;width:100%;max-height:90vh;overflow-y:auto;position:relative" onclick="event.stopPropagation()">
        <button onclick="document.getElementById('mo-anunt').remove()" style="position:absolute;top:12px;right:12px;background:rgba(0,0,0,0.6);border:none;color:#fff;width:32px;height:32px;border-radius:50%;cursor:pointer;font-size:18px;z-index:1">×</button>
        ${fotoSection}
        <div style="padding:20px">
          <div style="font-size:20px;font-weight:800;margin-bottom:4px">${a.brand||''} ${a.model||''} ${a.year||''}</div>
          <div style="font-size:14px;color:var(--t2);margin-bottom:12px">${a.km?Number(a.km).toLocaleString()+' km':''} ${a.combustibil?' · '+a.combustibil:''} ${a.motor?' · '+a.motor+'cm³':''}</div>
          ${a.descriere?`<div style="font-size:13px;color:var(--t2);line-height:1.6;margin-bottom:16px;padding:12px;background:var(--s2);border-radius:10px">${a.descriere}</div>`:''}
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px;font-size:13px">
            ${a.judet?`<div style="padding:8px;background:var(--s2);border-radius:8px">📍 ${a.localitate?a.localitate+', ':''}${a.judet}</div>`:''}
            ${a.vin?`<div style="padding:8px;background:var(--s2);border-radius:8px;font-family:monospace;font-size:11px">VIN: ${a.vin}</div>`:''}
          </div>
          ${a.indicatii?`<div style="font-size:12px;color:var(--t3);margin-bottom:12px">🗺️ ${a.indicatii}</div>`:''}
          <a href="tel:${a.tel}" style="display:block;background:var(--green);color:#000;font-weight:800;text-align:center;padding:14px;border-radius:12px;text-decoration:none;font-size:15px">📞 Sună — ${a.tel}</a>
          ${esteAlMeu?`<button onclick="document.getElementById('mo-anunt').remove()" style="width:100%;margin-top:8px;background:var(--s2);border:1px solid var(--b2);color:var(--t2);padding:10px;border-radius:10px;cursor:pointer;font-size:13px">Acesta e anunțul tău</button>`:''}
        </div>
      </div>
    </div>`;
