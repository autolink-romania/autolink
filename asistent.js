...
function newConversation() {
  ...
  // Arhivez conversația curentă înainte să o șterg
  const existing = JSON.parse(localStorage.getItem('chat_' + agent) || '[]');
  if(existing.length > 0) {
    const arhiva = JSON.parse(localStorage.getItem('chat_arhiva_' + agent) || '[]');
    arhiva.unshift({
      data: new Date().toLocaleString('ro-RO'),
      ts: Date.now(),
      mesaje: existing
    });
    // Păstrăm maxim 10 conversații arhivate
    if(arhiva.length > 10) arhiva.splice(10);
    localStorage.setItem('chat_arhiva_' + agent, JSON.stringify(arhiva));
  }
  // Acum șterg conversația curentă
  chatHistory = [];
  localStorage.removeItem('chat_' + agent);
  // În Supabase marcăm cu session_id nou, nu ștergem
  const msgs = document.getElementById('chat-msgs');
  ...
  showNotification('✅ Conversație nouă','Conversația anterioară a fost salvată în istoric.');
}
...
function showConvHistory() {
  ...
  Object.entries(agents).forEach(([key, label]) => {
    ...
    if(hist.length > 0) {
      ...
      html += `<div style="border:1px solid rgba(79,125,255,0.3);border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer" onmouseover="this.style.background='var(--s2)'" onmouseout="this.style.background=''" onclick="closeM('conv-history');document.querySelector('.achip[onclick*=\'${key}\']')?.click()">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
          <span style="font-weight:700">${label}</span>
          <span style="font-size:10px;background:rgba(79,125,255,0.15);color:var(--accent);padding:2px 8px;border-radius:10px">CURENTĂ · ${hist.length} msg</span>
        </div>
        <div style="font-size:12px;color:var(--t2)">${preview}</div>
      </div>`;
    }
    const arhiva = JSON.parse(localStorage.getItem('chat_arhiva_' + key) || '[]');
    arhiva.forEach((conv, i) => {
      if(!conv.mesaje||!conv.mesaje.length) return;
      hasAny = true;
      const lastMsg = conv.mesaje[conv.mesaje.length-1];
      const preview = (lastMsg.content||'').slice(0,80)+((lastMsg.content||'').length>80?'...':'');
      html += `<div style="border:1px solid var(--b2);border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer" onmouseover="this.style.background='var(--s2)'" onmouseout="this.style.background=''" onclick="showArchivedConv('${key}',${i})">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
          <span style="font-weight:700;color:var(--t2)">${label}</span>
          <span style="font-size:10px;color:var(--t3)">${conv.data} · ${conv.mesaje.length} msg</span>
        </div>
        <div style="font-size:12px;color:var(--t3)">${preview}</div>
      </div>`;
    });
  });
...
async function loadDashVanzari(){
  ...
  // Merge Supabase + localStorage (toate anunturile active)
  const localAnunturi = JSON.parse(localStorage.getItem('vanz_anunturi')||'[]').filter(a=>a.status==='activ');
  if(!anunturi.length && localAnunturi.length) {
    anunturi = localAnunturi.slice(-6).reverse();
  } else if(anunturi.length) {
    // Adaug si localAnunturi care nu sunt in Supabase (nu au supabase_id)
    const localOnly = localAnunturi.filter(l=>!l.supabase_id);
    anunturi = [...anunturi, ...localOnly].slice(0,6);
  }
    ...
    ${anunturi.map((a,ai)=>{
      ...
      return `<div onclick="showAnuntModal(window._dashAnunturi[${ai}])" style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:14px;overflow:hidden;cursor:pointer;transition:transform 0.2s" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
        <div style="height:110px;background:linear-gradient(135deg,rgba(79,125,255,0.12),rgba(200,150,12,0.08));display:flex;align-items:center;justify-content:center;position:relative;border-bottom:1px solid rgba(255,255,255,0.06);overflow:hidden">
          ${(a.fotos&&a.fotos[0])||a.foto?`<img src="${(a.fotos&&a.fotos[0])||a.foto}" style="width:100%;height:100%;object-fit:cover">`:`<div style="font-size:52px">${emoji}</div>`}
...
function showArchivedConv(agent, idx) {
  const arhiva = JSON.parse(localStorage.getItem('chat_arhiva_' + agent) || '[]');
  const conv = arhiva[idx];
  if(!conv) return;
  closeM('conv-history');
  // Schimb la agentul respectiv
  const chip = document.querySelector(`.achip[onclick*="'${agent}'"]`);
  if(chip) chip.click();
  setTimeout(()=>{
    const msgs = document.getElementById('chat-msgs');
    if(!msgs) return;
    const ICONS = {manager:'🧠',auto:'🔧',documente:'📄',juridic:'⚖️',vanzare:'💰',itp:'🔬'};
    const NAMES = {manager:'Manager AutoAssist',auto:'Auto & Service',documente:'Documente',juridic:'Juridic',vanzare:'Vânzare',itp:'ITP & Service'};
    msgs.innerHTML = `<div style="text-align:center;padding:8px;font-size:11px;color:var(--t3);margin-bottom:8px">📁 Conversație arhivată din ${conv.data}</div>` +
      conv.mesaje.map(m => {
        const isAi = m.role === 'assistant';
        return `<div class="msg ${isAi?'ai':'user'}">
          <div class="mav ${isAi?'ai':'user'}">${isAi?(ICONS[agent]||'🤖'):'Tu'}</div>
          <div class="mb"><div class="mn">${isAi?(NAMES[agent]||'Agent'):'Tu'}</div><div class="mbu">${m.content}</div></div>
        </div>`;
      }).join('');
    msgs.scrollTop = msgs.scrollHeight;
  }, 300);
