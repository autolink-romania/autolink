...
function selectVanzCar(id){
  ...
  // Precompletare date din garaj
  const pret = document.getElementById('vanz-pret');
  if(pret && !pret.value && c.pret_vanzare) pret.value = c.pret_vanzare;

  const tel = document.getElementById('vanz-tel');
  if(tel && !tel.value && c.tel) tel.value = c.tel;

  const judet = document.getElementById('vanz-judet');
  if(judet && !judet.value && c.judet) {
    judet.value = c.judet;
    vanzUpdateLocalitati();
    const loc = document.getElementById('vanz-localitate');
    if(loc && c.localitate) setTimeout(()=>{ loc.value = c.localitate; }, 50);
  }

  const desc = document.getElementById('vanz-desc');
  if(desc) desc.disabled = false; // siguranta - nu ramane blocat

  renderVanzGalerie();
  vanzEstimPret();
  // async - nu blochează UI
  setTimeout(()=>vanzRenderTarif(), 100);
}
