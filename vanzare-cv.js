...
async function selectVanzCar(id){
  selCarId = id;
  const c = cars.find(x=>x.id===id);
  if(!c) return;

  // Cauta anunt activ - din Supabase sau localStorage
  const anunturiSalvate = JSON.parse(localStorage.getItem('vanz_anunturi')||'[]');
  let anuntExistent = anunturiSalvate.find(a=>a.plate===c.plate&&a.status==='activ');
  // Cauta si in Supabase - mai multe variante de plate
  if(!anuntExistent && supabaseClient && currentUser) {
    try {
      const plateFormats = [c.plate, c.plate?.replace(/\s/g,''), c.plate?.toUpperCase()];
      for(const pf of plateFormats) {
        const {data} = await supabaseClient.from('listings').select('*').eq('plate',pf).eq('user_id',currentUser.id).eq('status','activ').limit(1);
        if(data && data[0]) { anuntExistent = data[0]; break; }
      }
      // Fallback - cauta doar dupa user_id si brand/model
      if(!anuntExistent) {
        const {data} = await supabaseClient.from('listings').select('*').eq('user_id',currentUser.id).eq('status','activ').order('created_at',{ascending:false}).limit(5);
        if(data) anuntExistent = data.find(a=>a.brand===c.brand&&a.model===c.model)||data[0]||null;
      }
    } catch(e) { console.error('Supabase listings search error:', e); }
  }
  ...
  localStorage.setItem('vanz_sel_car_id', id);

  // RESET complet toate câmpurile înainte de precompletare
  ['vanz-brand','vanz-model','vanz-an','vanz-km','vanz-culoare','vanz-combustibil',
   'vanz-motor','vanz-putere','vanz-vin','vanz-pret','vanz-tel','vanz-indicatii'].forEach(fid=>{
    const el=document.getElementById(fid); if(el) el.value='';
  });
  const descEl=document.getElementById('vanz-desc'); if(descEl){descEl.value='';descEl.disabled=false;}
  const cutieEl2=document.getElementById('vanz-cutie'); if(cutieEl2) cutieEl2.value='';
  const locEl=document.getElementById('vanz-localitate'); if(locEl) locEl.innerHTML='<option value="">Selectează mai întâi județul...</option>';
  const judetEl2=document.getElementById('vanz-judet'); if(judetEl2) judetEl2.value='';
  document.querySelectorAll('#vanz-dotari input').forEach(inp=>inp.checked=false);

  // Poze - din anunt existent sau din garaj
  window._vanzFotos = anuntExistent?.fotos?.filter(Boolean).length
    ? [...anuntExistent.fotos.filter(Boolean)]
    : (c.fotos?.filter(Boolean).length ? [...c.fotos.filter(Boolean)] : []);
  ...
  // Dacă există anunț activ - precompletez din anunț (forțat, indiferent de valoare)
  if(anuntExistent) {
    console.log('Precompletez din anunt:', anuntExistent);
    const _force=(fid,val)=>{const el=document.getElementById(fid);if(el&&val!=null&&val!=='')el.value=val;};
    _force('vanz-pret', anuntExistent.pret);
    _force('vanz-desc', anuntExistent.descriere);
    _force('vanz-tel', anuntExistent.tel);
    _force('vanz-culoare', anuntExistent.culoare||anuntExistent.color);
    _force('vanz-combustibil', anuntExistent.combustibil||anuntExistent.fuel);
    _force('vanz-motor', anuntExistent.motor||anuntExistent.engine);
    _force('vanz-putere', anuntExistent.putere||anuntExistent.power);
    _force('vanz-vin', anuntExistent.vin);
    _force('vanz-indicatii', anuntExistent.indicatii);
    const cutieAnunt = anuntExistent.cutie||anuntExistent.cutie_viteze;
    if(cutieAnunt){const el=document.getElementById('vanz-cutie');if(el)el.value=cutieAnunt;}
    const locNrAnunt = anuntExistent.locuri||anuntExistent.nr_locuri;
    if(locNrAnunt){const el=document.getElementById('vanz-locuri');if(el)el.value=locNrAnunt;}
    // Judet + localitate
    const judetAnunt = anuntExistent.judet||anuntExistent.county;
    if(judetAnunt) {
      const jEl=document.getElementById('vanz-judet');
      if(jEl){jEl.value=judetAnunt;vanzUpdateLocalitati();}
      setTimeout(()=>{
        const loc=document.getElementById('vanz-localitate');
        if(loc) loc.value=anuntExistent.localitate||anuntExistent.city||'';
      },100);
    }
    // Poze
    const fotosAnunt = anuntExistent.fotos||[];
    if(fotosAnunt.filter(Boolean).length) window._vanzFotos=[...fotosAnunt.filter(Boolean)];
    // Dotări
    if(anuntExistent.dotari) {
      anuntExistent.dotari.split(',').forEach(d=>{
        const d2=d.trim();
        document.querySelectorAll('#vanz-dotari input').forEach(inp=>{ if(inp.value===d2) inp.checked=true; });
      });
    }
  }
  ...
  if(!document.getElementById('vanz-lb')){
    document.body.insertAdjacentHTML('beforeend', `
      <div id="vanz-lb" style="position:fixed;inset:0;background:rgba(0,0,0,0.96);z-index:9999;display:flex;align-items:center;justify-content:center" onclick="if(event.target===this)document.getElementById('vanz-lb').remove()">
        ...
        <img id="vanz-lb-img" style="width:calc(100vw - 140px);height:88vh;border-radius:10px;object-fit:contain;display:block;background:#111">
...
async function vanzPublica(){
  ...
  // Dacă e a 2-a mașină (20 RON) și NU e modificare anunț existent
  const _areAnuntActiv = JSON.parse(localStorage.getItem('vanz_anunturi')||'[]')
    .some(a=>a.plate===cars.find(x=>x.id===selCarId)?.plate&&a.status==='activ');
  if(window._vanzCostPublicare===20 && !_areAnuntActiv){
  ...
  if(supabaseClient && currentUser){
    try {
      const supabasePayload = {
        ...
        brand: brand,
        model: model,
        year: parseInt(an)||null,
        km: parseInt(km)||null,
        pret: pret,
        combustibil: (document.getElementById('vanz-combustibil')||{}).value||null,
        motor: (document.getElementById('vanz-motor')||{}).value||null,
        putere: (document.getElementById('vanz-putere')||{}).value||null,
        cutie: (document.getElementById('vanz-cutie')||{}).value||null,
        culoare: (document.getElementById('vanz-culoare')||{}).value||null,
        locuri: (document.getElementById('vanz-locuri')||{}).value||null,
        vin: vin,
        descriere: desc,
        dotari: dotari,
        judet: judet,
        ...
        indicatii: indicatii,
        tel: tel,
        foto: (window._vanzFotos&&window._vanzFotos[0])||null,
        fotos: (window._vanzFotos||[]).filter(Boolean),
        status: 'activ'
      };
      const {data, error} = await supabaseClient.from('listings').insert([supabasePayload]).select('id');
      if(error) {
        console.error('Supabase listings error:', error);
        showNotification('⚠️ Avertisment', 'Anunțul e salvat local dar nu în cloud: ' + error.message);
      } else if(data && data[0]) {
        anunt.supabase_id = data[0].id;
      }
  ...
  if(typeof loadDashVanzari==='function') setTimeout(loadDashVanzari, 500);
...
function vanzNouAnunt(){
  ...
  localStorage.removeItem('vanz_sel_car_id');
...
async function vanzRenderTarif(){
  ...
  // Obtin anunturile userului
  let anunturiUser=[];
  if(supabaseClient && currentUser){
    try{
      const {data}=await supabaseClient.from('listings').select('id,created_at,status').eq('user_id',currentUser.id);
      if(data) anunturiUser=data;
    }catch(e){}
  }
  if(!anunturiUser.length){
    anunturiUser=JSON.parse(localStorage.getItem('vanz_anunturi')||'[]');
  }

  const anunturiActive = anunturiUser.filter(a=>a.status==='activ');
  const nrActiv = anunturiActive.length;

  // Verifică dacă primul anunț e în 7 zile gratuit
  const primaData = anunturiActive.length>0 ? new Date(anunturiActive[0].created_at||anunturiActive[0].data||Date.now()) : null;
  const zileDeAtunci = primaData ? Math.floor((Date.now()-primaData)/864e5) : 0;
  const esteInPerioadaGratuita = nrActiv===1 && zileDeAtunci<7;

  if(!prem && nrActiv===0){
    // Gratuit fără anunțuri — prima mașină gratuită
    el.style.background='rgba(0,232,154,0.07)';el.style.border='1px solid rgba(0,232,154,0.2)';
    el.innerHTML=`<div style="font-size:13px;font-weight:700;color:var(--green)">✅ Prima mașină — publicare GRATUITĂ 7 zile</div>
      <div style="font-size:12px;color:var(--t2);margin-top:4px">Publici prima ta mașină gratuit pe AutoAssist pentru 7 zile.</div>`;
    if(btn){btn.textContent='🚀 Publică Anunțul — GRATUIT 7 zile';btn.disabled=false;btn.style.opacity='1';}
    window._vanzCostPublicare=0;
  } else if(!prem && nrActiv>0 && !esteInPerioadaGratuita){
    // Gratuit expirat sau mai multe mașini fără premium
    el.style.background='rgba(255,184,48,0.07)';el.style.border='1px solid rgba(255,184,48,0.25)';
    el.innerHTML=`<div style="font-size:13px;font-weight:700;color:var(--amber)">⭐ Promovare necesară</div>
      <div style="font-size:12px;color:var(--t2);margin-top:4px">Perioada gratuită a expirat. Promovează cu <strong>20 RON / 7 zile</strong> sau activează Premium pentru anunțuri nelimitate.</div>
      <div style="display:flex;gap:8px;margin-top:8px">
        <button class="btn btn-ghost btn-sm" onclick="showUpgradeModal()" style="flex:1">👑 Premium 49 RON/an</button>
      </div>`;
    if(btn){btn.textContent='💳 Promovează — 20 RON / 7 zile';btn.disabled=false;btn.style.opacity='1';}
    window._vanzCostPublicare=20;
  } else if(prem && nrActiv===0){
    // Premium fără anunțuri
    el.style.background='rgba(0,232,154,0.07)';el.style.border='1px solid rgba(0,232,154,0.2)';
    el.innerHTML=`<div style="font-size:13px;font-weight:700;color:var(--green)">✅ Premium activ — publicare inclusă</div>
      <div style="font-size:12px;color:var(--t2);margin-top:4px">Prima mașină gratuită, anunțuri nelimitate cu Premium.</div>`;
    if(btn){btn.textContent='🚀 Publică Anunțul Acum — GRATUIT';btn.disabled=false;btn.style.opacity='1';}
    window._vanzCostPublicare=0;
  } else {
    // Premium sau prima perioadă gratuită - mașini suplimentare 20 RON
    el.style.background='rgba(255,184,48,0.07)';el.style.border='1px solid rgba(255,184,48,0.25)';
    const zileRamasite = esteInPerioadaGratuita ? (7-zileDeAtunci) : 0;
    el.innerHTML=`<div style="font-size:13px;font-weight:700;color:var(--amber)">💳 Mașină suplimentară — 20 RON / 7 zile</div>
      <div style="font-size:12px;color:var(--t2);margin-top:4px">Ai deja ${nrActiv} anunț${nrActiv>1?'uri':''} activ${nrActiv>1?'e':''}. Fiecare mașină suplimentară costă <strong>20 RON</strong> pentru 7 zile promovare.</div>
      <div style="font-size:11px;color:var(--t3);margin-top:4px">Plata disponibilă după lansarea sistemului de plăți Stripe.</div>`;
...
async function vanzLoadLista(){
  ...
  el.innerHTML=anunturi.map(a=>`
    <div style="border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:12px;margin-bottom:10px">
      ...
      <div style="display:flex;gap:6px;margin-top:8px">
        <button onclick="showAnuntModal(window._vanzListaAnunturi.find(x=>String(x.id)===String('${a.id}')))" style="flex:1;font-size:11px;font-weight:700;padding:5px;background:rgba(79,125,255,0.1);color:var(--accent);border:none;border-radius:6px;cursor:pointer">👁️ Vezi</button>
  ...
  window._vanzListaAnunturi = anunturi;
}
