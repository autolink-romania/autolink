...
function selectVanzCar(id){
  ...
  // Cauta anunt activ pentru aceasta masina
  const anunturiSalvate = JSON.parse(localStorage.getItem('vanz_anunturi')||'[]');
  const anuntExistent = anunturiSalvate.find(a=>a.plate===c.plate&&a.status==='activ');
  ...
  const fotosSursa = anuntExistent?.fotos?.length ? anuntExistent.fotos : (c.fotos?.length ? [...c.fotos] : (_savedFotos.length ? _savedFotos : []));
  window._vanzFotos = fotosSursa;
