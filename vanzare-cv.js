...
async function selectVanzCar(id){
  ...
  // Cauta si in Supabase
  if(!anuntExistent && supabaseClient && currentUser) {
    try {
      const {data} = await supabaseClient.from('listings').select('*').eq('plate',c.plate).eq('user_id',currentUser.id).eq('status','activ').limit(1);
      if(data && data[0]) anuntExistent = data[0];
    } catch(e) {}
  }
  ...
  // Dacă există anunț activ - precompletez din anunț
  if(anuntExistent) {
    _set('vanz-pret', anuntExistent.pret);
    _set('vanz-desc', anuntExistent.descriere);
    _set('vanz-tel', anuntExistent.tel);
    _set('vanz-culoare', anuntExistent.culoare);
    _set('vanz-combustibil', anuntExistent.combustibil);
    _set('vanz-motor', anuntExistent.motor);
    _set('vanz-putere', anuntExistent.putere);
    _set('vanz-vin', anuntExistent.vin);
    _set('vanz-indicatii', anuntExistent.indicatii);
    if(document.getElementById('vanz-judet')) {
      document.getElementById('vanz-judet').value = anuntExistent.judet||'';
      vanzUpdateLocalitati();
      setTimeout(()=>{
        const loc=document.getElementById('vanz-localitate');
        if(loc) loc.value=anuntExistent.localitate||'';
      },60);
    }
    if(anuntExistent.fotos&&anuntExistent.fotos.length) {
      window._vanzFotos = [...anuntExistent.fotos];
    }
    // Dotări bifate
